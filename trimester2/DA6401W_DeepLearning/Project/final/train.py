...
class EmojiDataset(Dataset):
    def __init__(self, csv_path: Path):
        ...
        self.df = self.df.drop_duplicates(
            ["Emoji", "Description"]
        )  # shouldn't be required, but just in case train data has any duplicates accidentally
        ...
        self.labels = torch.tensor(
            [self.emoji_to_idx[e] for e in self.df["Emoji"]], dtype=torch.long
        )
...
class EmojiClassifier(L.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        avg_top5 = (
            torch.stack([x["top5_acc"] for x in self.test_step_outputs]).mean().item()
        )
...
if __name__ == "__main__":
    ...
    model = EmojiClassifier(
        input_dim=768, num_classes=len(train_ds.emojis), emojis=emojis
    )
    ...
    print(
        f"Final Weights Saved to {best_model_path} (Best Val Acc: {checkpoint_callback.best_model_score:.4f})"
    )
