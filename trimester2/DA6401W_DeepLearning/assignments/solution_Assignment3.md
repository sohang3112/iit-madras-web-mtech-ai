...
## Problem 1: Explore what causes slow training in multilayer perceptron neural networks even in the absence of vanishing gradients (a statistical view)
...
**Problem Setup**
    ...
    - Dataset: MNIST or Fashion-MNIST ( you can import dataset from torchvision.datasets.MNIST)


   - Input shape: 28 \times 28


    - 28 \times 28 grayscale images, flattened to 784
...
Use a simple, deliberately unstable model, below is an example
...
Choose one fixed mini-batch from the training set.


At the end of each training epoch / fixed number of gradient update steps:


**Run a forward pass on this fixed batch**


Using Pytorch's forward hooks, record activations (output of activation layer) from each
hidden layer, of shape (batch size, num channel):


**Visualization Requirements:**


**Histogram Evolution** At the end of each training epoch, flatten the activation tensor of
shape ( _B, C_ ) into a one-dimensional vector of size _B  \times  C_ and plot its histogram.


The histogram should be updated after every epoch. This can be done by either:


   - saving one histogram figure per epoch, or


   - creating an animation that shows the evolution of the activation distribution over time.


**Mean and Variance Over Time**


For each hidden layer and at every epoch, compute:


   - the mean of the activations, and


   - the variance of the activations.


Plot the mean versus training epoch and the variance versus training epoch. Each hidden
layer should have its own set of plots.
...
Implement the Adam optimizer from scratch and compare it with vanilla Stochastic Gradient
Descent (SGD) on a non-convex optimization problem.


**Problem Statement**


Consider the following non-convex function (Rosenbrock function):


_f_ ( _x, y_ ) = _x_ [2] + 100( _y -_ _x_ [2] ) [2]
...
2. Use the same learning rate 10 _[-]_ [3] initialization for both methods.


3. Optimize the function starting from the initial point:


( _x_ 0 _, y_ 0) = ( _-_ 1 _._ 5 _,_ 1 _._ 5)
...
6. Compare the two methods in terms of:
...
\eta \in {_ 10 _[-]_ [1] _,_ 10 _[-]_ [2] _,_ 10 _[-]_ [3] _,_ 10 _[-]_ [4] _}_
...
## Problem 6: Programming + Analysis: RMSProp vs Adam


Consider the non-convex function:


_f_ ( _w_ 1 _, w_ 2) = [1] 1 [+ 1000] _[w]_ 2 [2][) + 5 sin(] _[w]_ [1][)]
...
## Problem 8: Coding and Analysis 

Empirically validate the need for specific weight initialization strategies.


Task: Build a 2-layer Multilayer Perceptron (MLP) without using high-level framework layers (e.g., use NumPy matrix multiplications). Pass a batch of normally distributed inputs
through the network. Record and plot the variance of the activations (plot the histogram
of each neurons activation value) at each layer using four different initialization schemes:
Zero initialization, standard random normal initialization, Xavier initialization for Sigmoid
activations ( _N_ (0 _,_ 2 2
_nin_ + _nout_ [)][), and Kaiming He initialization for ReLU activations (] _[N]_ [(0] _[,]_ _nin_ [)][)]
...
**Network configuration**
   ...
   - Hidden layer sizes (layers 1, 2): 5


   - Output layer size: 5 (no activation; just for completeness as we are not training here)

|Initialization Type|Network 1 (Hidden Activation)|Network 2 (Hidden Activation)|
|---|---|---|
|Zero|ReLU|Sigmoid|
|Standard Normal|ReLU|Sigmoid|
|Xavier (Glorot)|Sigmoid|-|
|Kaiming (He)|ReLU|-|



Table 1: Neural networks to build for this exercise


Total 6 networks (See Table 1). Visualise the activation distribution as a histogram for
each neuron in the entire network and provide your justification on the effects of weight
initialisation. It will be 128 (batch size) values per neuron, and make a histogram of it for
each hidden neuron.
...
## Problem 10: Coding: RMSProp with ReLU Activation
...
**Where:**
 ...
 - _w_ : weight parameter

 - _gt_ = _ \parital w_ _[ \parital L]_ [: gradient]
...
_vt_ = \beta vt-_ 1 + (1 - \beta ) _gt_ [2]


\eta
_wt_ +1 = _wt -_ ~~_\sqrt_~~ _t_
~~_v_~~ _t_ + \epsilon g_
...
## Problem 12: Coding: Optimization in a Non-Convex Loss


Consider the non-convex function


_L_ ( _x_ ) = _x_ [4] _-_ 4 _x_ [2] + _x._


1. Plot _L_ ( _x_ ) for _x  \in _ [ _-_ 3 _,_ 3] and identify the number of local minima.


2. Implement Gradient Descent and Adam from scratch.


3. Run both optimizers for 300 iterations from:


_x_ 0 = _-_ 1 _._ 5 and _x_ 0 = +1 _._ 5 _._
...
## Problem 14: Programming : Comparing SGD and Momentum


Consider minimizing the function:


_f_ ( _w_ 1 _, w_ 2) = [1] 1 [+] _[ w]_ 2 [2][)]
...
(c) Report:
      ...
      - Plot trajectory in ( _w_ 1 _, w_ 2) space
...
## Problem 15: Programming: Gradient Descent vs Newton's Method


Consider the function


_f_ ( _w_ 1 _, w_ 2) = 3 _w_ 1 [2] [+ 2] _[w]_ [1] _[w]_ [2] [+ 2] _[w]_ 2 [2]


The loss value at iteration _t_ is defined as


_Lt_ = _f_                   - _w_ 1 [(] _[t]_ [)] _[, w]_ 2 [(] _[t]_ [)]                   

where _w_ 1 [(] _[t]_ [)] and _w_ 2 [(] _[t]_ [)] denote the parameter values at iteration _t_ .


The Hessian of this function is constant and positive definite, therefore the function is convex
...
1. Derive the gradient vector \nablaf_ ( _w_ ) and the Hessian matrix \nabla_ [2] _f_ ( _w_ ).


2. Implement the following optimization algorithms. Use PyTorch, TensorFlow, or built-in
optimizer libraries.
...
3. Initialize the parameters as
...
5. Use the following Newton update rule (without a learning rate):


_wt_ +1 = _wt -_ ( \nabla_ [2] _f_ ) _[-]_ [1] \nablaf_ ( _wt_ )


6. Run Gradient Descent for **50 iterations** and Newton's Method for **5 iterations** .


7. Plot


     - Loss _Lt_ versus iteration number

      - Optimization trajectory in the ( _w_ 1 _, w_ 2) plane


8. Briefly compare the convergence behavior of the two methods.
