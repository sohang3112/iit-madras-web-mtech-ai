...
NEW --> covariance matrix of weights $C = Cov(\beta, \beta) = \sigma^2 (X^T X)^{-1} $ where $\sigma^2$ is noise aka variance of error in matrix equation $Var(\epsilon)$
* covariance matrix of weights $C$ is *symmetric matrix* since $X^T X$ is symmetric
* true $\sigma^2$ is unknown, so we must estimate it: $
\hat{\sigma}^2 = SSE / (n-p)$ where $n$ is no. of samples/rows, $p$ is no. of parameters/weights (size of covariance matrix of weights)
* **Estimated Standard Error** (after using this estimate of error variance) is $SE[\hat{\beta_j}] = \sqrt{\hat{\sigma}^2 C_{jj}}$. Small errors for good precision.
...
$$(x - \mu)^T \Sigma^{-1} (x - \mu) = d^2 \le \chi^2 (\alpha) \quad \text{with probability } 1-\alpha$$
...
- value of $d^2$ at $100 (1 - \alpha)$ 'th percentile represents all multivariate observations of X such that $100 (1 - \alpha) %$ lie below curve..
